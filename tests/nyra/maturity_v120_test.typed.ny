// Maturity improvements v1.23 — explicit types
...
test fn test_json_str_array_roundtrip() {
    let mut tags: StrVec = StrVec_new()
    tags = tags.push("alpha")
    tags = tags.push("beta")
    let json: string = json_encode_str_array(StrVec_raw(tags))
    let h: ptr = json_decode_str_array(json)
...
test fn test_struct_serde_with_name() {
    let s: TagList = TagList { name: "demo" }
    let json: string = TagList_json_encode(s)
    let s2: TagList = TagList_json_decode(json)
    assert_str_eq(s2.name, "demo")
}

test fn test_matrix2d_put_get() {
    let mut m: Matrix2D = Matrix2D_new(2, 3)
    m = m.put(0, 1, 7)
    m = m.put(1, 2, 9)
