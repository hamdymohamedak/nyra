...
ENTRIES: list[dict[str, Any]] = [
    ...
    # ══════════════════════════════════════════════════════════════════════
    # Gap fill — RPC / data / observability / wasm / media live / auth / cloud
    # ══════════════════════════════════════════════════════════════════════
    # --- RPC & serialization ---
    _e("grpc", "gRPC C core", ["grpc/grpc.h"], ["grpc"], pkg_config="grpc", brew="grpc", apt="libgrpc-dev", pacman="grpc", dnf="grpc-devel", aliases=["grpc-c"]),
    _e("grpcpp", "gRPC C++ API", ["grpcpp/grpcpp.h"], ["grpc++"], pkg_config="grpc++", brew="grpc", apt="libgrpc++-dev", pacman="grpc", dnf="grpc-devel", aliases=["grpc-cpp"], depends=["grpc", "protobuf", "abseil"], cxx=True),
    _e("thrift", "Apache Thrift RPC/serialization", ["thrift/Thrift.h"], ["thrift"], pkg_config="thrift", brew="thrift", apt="libthrift-dev", pacman="thrift", dnf="thrift-devel", cxx=True),
    _e("capnp", "Cap'n Proto serialization/RPC", ["capnp/message.h"], ["capnp", "kj"], pkg_config="capnp", brew="capnp", apt="libcapnp-dev", pacman="capnproto", dnf="capnproto-devel", aliases=["capnproto"], cxx=True),
    # --- Analytics / columnar ---
    _e("arrow", "Apache Arrow columnar in-memory", ["arrow/api.h"], ["arrow"], pkg_config="arrow", brew="apache-arrow", apt="libarrow-dev", pacman="arrow", dnf="libarrow-devel", aliases=["apache-arrow", "libarrow"], cxx=True),
    _e("parquet", "Apache Parquet (via Arrow)", ["parquet/arrow/reader.h"], ["parquet"], pkg_config="parquet", brew="apache-arrow", apt="libparquet-dev", pacman="arrow", dnf="libparquet-devel", aliases=["libparquet"], depends=["arrow"], cxx=True),
    _e("nanoarrow", "Arrow C data interface (nanoarrow)", ["nanoarrow/nanoarrow.h"], ["nanoarrow"], brew="nanoarrow", apt="-", pacman="-", dnf="-", git="https://github.com/apache/arrow-nanoarrow.git"),
    _e("duckdb", "DuckDB embedded analytics DB", ["duckdb.h"], ["duckdb"], brew="duckdb", apt="libduckdb-dev", pacman="duckdb", dnf="duckdb-devel"),
    # --- Observability ---
    _e("prometheus-cpp", "Prometheus metrics C++ client", ["prometheus/counter.h"], ["prometheus-cpp-core", "prometheus-cpp-pull"], brew="prometheus-cpp", apt="-", pacman="prometheus-cpp", dnf="-", aliases=["prometheus"], cxx=True),
    _e(
        "opentelemetry-c",
        "OpenTelemetry C API (experimental)",
        ["opentelemetry/c/trace.h"],
        ["opentelemetry_common"],
        brew="-",
        apt="-",
        pacman="-",
        git="https://github.com/open-telemetry/opentelemetry-c.git",
        aliases=["otel-c"],
    ),
    _e(
        "opentelemetry-cpp",
        "OpenTelemetry C++ API/SDK",
        ["opentelemetry/exporters/ostream/span_exporter.h"],
        ["opentelemetry_trace", "opentelemetry_common"],
        brew="opentelemetry-cpp",
        apt="libopentelemetry-dev",
        pacman="opentelemetry-cpp",
        dnf="opentelemetry-cpp-devel",
        aliases=["otel-cpp", "opentelemetry"],
        cxx=True,
    ),
    # --- WebAssembly ---
    _e("wasmtime", "Wasmtime Wasm runtime (C API)", ["wasm.h"], ["wasmtime"], brew="wasmtime", apt="-", pacman="wasmtime", dnf="-", aliases=["wasmtime-c"]),
    _e("wasmer", "Wasmer Wasm runtime (C API)", ["wasmer.h"], ["wasmer"], brew="wasmer", apt="-", pacman="wasmer", dnf="-"),
    _e(
        "wamr",
        ...
        apt="-",
        pacman="wasm-micro-runtime",
        dnf="-",
        aliases=["wasm-micro-runtime"],
    ),
    # --- Live media ---
    _e("gstreamer", "GStreamer multimedia framework", ["gst/gst.h"], ["gstreamer-1.0"], pkg_config="gstreamer-1.0", brew="gstreamer", apt="libgstreamer1.0-dev", pacman="gstreamer", dnf="gstreamer1-devel", aliases=["gst"]),
    _e(
        "gst-plugins-base",
        "GStreamer base plugins (app/audio/video)",
        ["gst/app/gstappsink.h"],
        ["gstapp-1.0", "gstaudio-1.0", "gstvideo-1.0"],
        pkg_config="gstreamer-app-1.0",
        brew="gstreamer",
        apt="libgstreamer-plugins-base1.0-dev",
        pacman="gst-plugins-base",
        dnf="gstreamer1-plugins-base-devel",
        depends=["gstreamer"],
        aliases=["gstreamer-plugins-base"],
    ),
    _e(
        "libwebrtc",
        "Google WebRTC native (C++ — large; git)",
        ["api/peer_connection_interface.h"],
        ["webrtc"],
        brew="-",
        apt="-",
        pacman="-",
        git="https://github.com/webrtc-uwp/webrtc-deps.git",
        aliases=["webrtc"],
        cxx=True,
    ),
    # --- Auth / enterprise ---
    _e("openldap", "OpenLDAP client libraries", ["ldap.h"], ["ldap", "lber"], pkg_config="ldap", brew="openldap", apt="libldap2-dev", pacman="libldap", dnf="openldap-devel", aliases=["ldap"]),
    _e("krb5", "Kerberos 5 (MIT)", ["krb5.h"], ["krb5"], pkg_config="krb5", brew="krb5", apt="libkrb5-dev", pacman="krb5", dnf="krb5-devel", aliases=["kerberos", "gssapi"]),
    _e("cyrus-sasl", "Cyrus SASL authentication", ["sasl/sasl.h"], ["sasl2"], pkg_config="libsasl2", brew="cyrus-sasl", apt="libsasl2-dev", pacman="cyrus-sasl", dnf="cyrus-sasl-devel", aliases=["sasl", "libsasl2"]),
    _e(
        "pam",
    ...
    _e("pkcs11-helper", "PKCS#11 helper library", ["pkcs11-helper-1.0/pkcs11h.h"], ["pkcs11-helper"], pkg_config="libpkcs11-helper-1", brew="pkcs11-helper", apt="libpkcs11-helper1-dev", pacman="pkcs11-helper", dnf="pkcs11-helper-devel", aliases=["pkcs11"]),
    _e(
        "tpm2-tss",
        "TPM 2.0 Software Stack (TSS)",
        ["tss2/tss2_esys.h"],
        ["tss2-esys", "tss2-mu", "tss2-sys"],
        brew="-",
        apt="libtss2-dev",
        pacman="tpm2-tss",
        dnf="tpm2-tss-devel",
    ...
    _e(
        "wireguard",
        "WireGuard Linux kernel UAPI headers",
        ["linux/wireguard.h"],
        ["m"],
        brew="-",
        apt="linux-libc-dev",
        ...
        dnf="kernel-headers",
        platforms=["linux"],
        aliases=["wireguard-linux"],
    ),
    # --- Cloud SDKs ---
    _e("aws-c-common", "AWS C common utilities", ["aws/common/common.h"], ["aws-c-common"], pkg_config="aws-c-common", brew="aws-c-common", apt="-", pacman="aws-c-common", dnf="-"),
    _e("aws-c-s3", "AWS C S3 client", ["aws/s3/s3_client.h"], ["aws-c-s3"], brew="aws-c-s3", apt="-", pacman="-", dnf="-", depends=["aws-c-common"]),
    _e(
        "aws-sdk-cpp",
        "AWS SDK for C++ (core)",
        ["aws/core/Aws.h"],
    ...
    _e(
        ...
        aliases=["azure-iot", "azure-iot-sdk"],
    ),
    # --- Robotics / 3D ---
    _e("pcl", "Point Cloud Library", ["pcl/point_cloud.h"], ["pcl_common"], pkg_config="pcl_common", brew="pcl", apt="libpcl-dev", pacman="pcl", dnf="pcl-devel", aliases=["pointcloud"], cxx=True),
    _e("realsense", "Intel RealSense SDK 2", ["librealsense2/rs.h"], ["realsense2"], pkg_config="realsense2", brew="librealsense", apt="librealsense2-dev", pacman="librealsense", dnf="-", aliases=["librealsense", "librealsense2"]),
    _e("bullet", "Bullet Physics SDK", ["btBulletDynamicsCommon.h"], ["BulletDynamics", "BulletCollision", "LinearMath"], brew="bullet", apt="libbullet-dev", pacman="bullet", dnf="bullet-devel", aliases=["bullet3"], cxx=True),
    _e(
        "rclc",
        "ROS 2 client library (rclc) — C API",
        ["rclc/rclc.h"],
        ["rclc"],
        brew="-",
        apt="ros-humble-rclc",
        ...
        dnf="-",
        platforms=["linux"],
        aliases=["ros2-rclc", "ros2"],
        git="https://github.com/ros2/rclc.git",
    ),
    # --- Documents / barcodes ---
    _e(
        "pdfium",
        "PDFium PDF engine (Chromium)",
        ["fpdfview.h"],
        ["pdfium"],
        brew="-",
        apt="-",
        pacman="-",
        git="https://github.com/bblanchon/pdfium-binaries.git",
        aliases=["pdfium-binaries"],
    ),
    _e("zbar", "ZBar barcode reader", ["zbar.h"], ["zbar"], pkg_config="zbar", brew="zbar", apt="libzbar-dev", pacman="zbar", dnf="zbar-devel"),
    _e("zxing-cpp", "ZXing barcode (C++)", ["ZXing/ReadBarcode.h"], ["ZXing"], pkg_config="zxing", brew="zxing-cpp", apt="libzxing-dev", pacman="zxing-cpp", dnf="zxing-cpp-devel", aliases=["zxing"], cxx=True),
    # --- AI extras ---
    _e("whisper-cpp", "OpenAI Whisper (C/C++ local)", ["whisper.h"], ["whisper"], brew="whisper-cpp", apt="-", pacman="whisper-cpp", dnf="-", aliases=["whisper", "whisper.cpp"]),
    _e(
        "ctranslate2",
        "CTranslate2 fast Transformer inference",
        ["ctranslate2/translator.h"],
        ["ctranslate2"],
        brew="-",
        apt="-",
        pacman="ctranslate2",
    ...
    # --- Messaging extras ---
    _e("rabbitmq-c", "RabbitMQ AMQP 0-9-1 C client", ["amqp.h"], ["rabbitmq"], pkg_config="librabbitmq", brew="rabbitmq-c", apt="librabbitmq-dev", pacman="rabbitmq-c", dnf="librabbitmq-devel", aliases=["amqp", "librabbitmq"]),
    _e("qpid-proton", "Apache Qpid Proton AMQP 1.0", ["proton/message.h"], ["qpid-proton"], pkg_config="libqpid-proton", brew="qpid-proton", apt="libqpid-proton11-dev", pacman="qpid-proton", dnf="qpid-proton-c-devel", aliases=["proton", "amqp1"]),
    _e(
        "nats-c",
        "NATS.io C client",
        ["nats.h"],
        ...
        pacman="-",
        dnf="-",
        git="https://github.com/nats-io/nats.c.git",
        aliases=["nats", "cnats", "nats.c"],
    ),
    _e("pulsar", "Apache Pulsar C++ client", ["pulsar/Client.h"], ["pulsar"], brew="libpulsar", apt="libpulsar-dev", pacman="-", dnf="-", aliases=["libpulsar", "apache-pulsar"], cxx=True),
    # --- Extra DB clients ---
    _e(
        "cassandra",
        ...
        apt="libcassandra-dev",
        pacman="-",
        dnf="cassandra-cpp-driver-devel",
        aliases=["cassandra-cpp-driver", "datastax-cpp"],
    ),
    _e(
        "neo4j-client",
        "libneo4j-client (Bolt C client)",
        ["neo4j-client.h"],
        ["neo4j-client"],
        brew="-",
        apt="libneo4j-client-dev",
        pacman="-",
        dnf="-",
        git="https://github.com/cleishm/libneo4j-client.git",
        aliases=["neo4j", "libneo4j-client"],
    ),
    _e(
        "elasticsearch-c",
        "Elasticsearch official C client (elastic/elasticsearch-c is limited; use curl+JSON often)",
        ["elastic/client.h"],
        ["elasticsearch"],
        brew="-",
        apt="-",
        pacman="-",
        git="https://github.com/elastic/elasticsearch-c.git",
        aliases=["elasticsearch", "elastic-c"],
    ),
    # --- GPU / Apple / Intel extras ---
    _e(
        "moltenvk",
        "MoltenVK — Vulkan on Metal (macOS)",
    ...
    _e(
        "mkl",
        "Intel oneMKL (Linux packages / Intel channel)",
        ["mkl.h"],
        ["mkl_rt"],
        brew="-",
        apt="intel-mkl",
        pacman="intel-oneapi-mkl",
        dnf="intel-oneapi-mkl",
        platforms=["linux"],
        aliases=["onemkl", "intel-mkl"],
    ),
    _e(
        "onedal",
        ...
        brew="-",
        apt="-",
        pacman="intel-oneapi-dal",
        dnf="intel-oneapi-dal",
        platforms=["linux"],
        aliases=["dal", "intel-dal"],
        cxx=True,
    ),
]
