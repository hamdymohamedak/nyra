...
GUIDES: dict[str, RecipeGuide] = {
    ...
    "c-lib-registry": RecipeGuide(
        slug="c-lib-registry",
        title="C Library Registry Entry",
        when="Add `nyra bind <name>` support for a system C library (Homebrew/apt/dnf/pacman).",
        steps=(
            WizardStep(
                "Nyra bind name (also default brew formula)",
                "Users type this after: nyra bind <name>. Prefer the Homebrew formula name.",
                "Writes registry/c/<name>.toml and wires cli/src/c_registry.rs.",
                "Nothing else — OS package manager installs the real library.",
                "libsodium",
                "libsodium",
            ),
            WizardStep(
                "Short description (optional — Enter to skip)",
                "Shown in docs / contributor lists.",
                "Stored as description = \"…\" in the toml.",
                "Keep it one line.",
                "Modern encryption library (NaCl)",
                "",
            ),
            WizardStep(
                "Primary C header path inside the package",
                "Relative to include/ (what clang opens). Examples: sodium.h, gsl/gsl_sf.h.",
                "Used by bindgen after pkg-config / brew --prefix discovery.",
                "Verify with: ls $(brew --prefix <formula>)/include/…",
                "sodium.h",
                "sodium.h",
            ),
            WizardStep(
                "Link library name(s), comma-separated (no lib prefix)",
                "Goes into nyra.mod as `link <name>`. Often shorter than the brew formula.",
                "Writes libs = […] in the toml.",
                "Check: brew --prefix <formula>/lib/lib*.dylib → drop the lib prefix.",
                "sodium   (not libsodium)",
                "sodium",
            ),
            WizardStep(
                "pkg-config module name (optional — Enter to skip)",
                "If the package ships a .pc file, Nyra uses pkg-config --cflags/--libs.",
                "Writes pkg_config = \"…\". Skip if no .pc (Nyra falls back to include search).",
                "Try: pkg-config --exists <name> && echo ok",
                "libsodium",
                "",
            ),
            WizardStep(
                "Homebrew formula (Enter = same as name)",
                "Must match `brew install <formula>`.",
                "Used when suggesting/running brew install if the lib is missing.",
                "Confirm: brew info <formula>",
                "libsodium",
                "",
            ),
            WizardStep(
                "apt package (optional — Enter to skip)",
                "Debian/Ubuntu -dev package. Skip if unknown or not on apt.",
                "Only written if you provide a value — skipping is fine.",
                "Search: apt-cache search libsodium | head",
                "libsodium-dev",
                "",
            ),
            WizardStep(
                "dnf package (optional — Enter to skip)",
                "Fedora/RHEL -devel package. Skip if unknown.",
                "Only written if set.",
                "Often <name>-devel",
                "libsodium-devel",
                "",
            ...
            WizardStep(
                "pacman package (optional — Enter to skip)",
                "Arch Linux package name. Skip if unknown.",
                "Only written if set.",
                "Often same as the brew/formula name",
                "libsodium",
                "",
            ),
            WizardStep(
                "Aliases, comma-separated (optional — Enter to skip)",
                "Extra names that resolve to this entry (e.g. z → zlib).",
                "Writes aliases = […].",
                "Keep short; avoid colliding with other registry names.",
                "sodium",
                "",
            ),
        ),
        tool_files="""    • registry/c/<name>.toml              — package-name map [contrib-dev:…]
    • cli/src/c_registry.rs               — BUILTIN include_str! wiring""",
        you_files="""    • Install via brew/apt and run: nyra bind <name>
    • Optionally fill skipped apt/dnf/pacman later
    • No C code to write — only registry metadata""",
        verify="brew install <brew> && nyra bind <name> && nyra run .",
    ),
}
