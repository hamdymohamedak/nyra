...
pub fn c_add(name: &str, opts: AddOptions) -> Result<(), String> {
    ...
    if let Some(define) = entry.impl_define.as_deref() {
        ensure_header_impl_shim(&root, &entry, define, &header)?;
    }
...
fn ensure_header_impl_shim(
    ...
    let header_name = header
        .file_name()
        .and_then(|n| n.to_str())
        .ok_or_else(|| format!("invalid header path: {}", header.display()))?;
    let shim_dir = header
        .parent()
        .ok_or_else(|| format!("header has no parent: {}", header.display()))?;
    let shim_name = format!("{}_impl.c", entry.name);
    let shim_path = shim_dir.join(&shim_name);
    let body = format!(
        "/* Auto-generated by `nyra pkg add {}` — do not edit. */\n#define {define}\n#include \"{header_name}\"\n",
        entry.name
    );
    std::fs::create_dir_all(shim_dir).map_err(|e| e.to_string())?;
    std::fs::write(&shim_path, body).map_err(|e| e.to_string())?;

    let rel = pathdiff_rel(root, &shim_path);
    apply_nyra_mod_link_source(root, &rel)?;
    eprintln!(
        "{}",
        Ui::new().dim(&format!("link-source {rel}  ({define})"))
    );
    Ok(())
}
...
fn apply_nyra_mod_link_source(root: &Path, rel: &str) -> Result<(), String> {
    ...
    let line = format!("link-source {rel}");
